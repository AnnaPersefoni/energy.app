...
def editing_features_training():
    ...
    # data = pd.read_excel('excel/exNo.xlsx',usecols=cols,skiprows=1)
    data = pd.read_excel('excel/exNo.xlsx')
    ...
    df = train0
    target = df['Kwh/day/m2']
    df = df.drop(columns=['Kwh/day/m2'])
    ...
    new_df1 = pd.DataFrame(scale, columns=['Dwelling Grade', 'Size Grade', 'Bedrooms', 'Old',
                                       'Area Code', 'Ainc', 'Adec', 'Agauge', 'Fulltimers',
                                       'Parttimers', 'Grads', 'PostGrads', 'Education Index',
                                       'Income', 'Recycling', 'Energy Class', 'Thermostats',
                                       'Smart Plugs', 'Awareness', 'Heating Source_Yes', 'Water Heater_Yes'])
    df1 = new_df1
    ...
    # df1 = df1.drop(columns=['Area Code', 'Ainc', 'Adec', 'Grads', 'PostGrads', 'Education Index',
    #                         'Smart Plugs', 'Awareness', 'Heating Source_Yes'])


    # print("Traing transformation data ")
    # print(df1)

    X_array_test = np.array(df1)
    y_array_test = np.array(target)

    return X_array_test, y_array_test
